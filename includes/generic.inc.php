...
{
	$CON = mysqli_connect(DB_HOST, DB_USERNAME, DB_PASSWORD) or die(mysqli_connect_error() . "<strong>ERROR CODE : </strong> COM - 66");
...
function GetStatusImageString($mode, $status, $id, $ajax_flag = true)
{
	$str = "";
	if ($ajax_flag) {
		if ($status == "A") $str = '<button type="button" class="" onClick="ChangeStatus(this, \'' . $mode . '\',\'I\',\'' . $id . '\'); return false;"><i class="bx bx-check"></i></button>';
		else if ($status == 'P') $str = '<button class="" onClick="ChangeStatus(this, \'' . $mode . '\',\'A\',\'' . $id . '\'); return false;">P</button>';
		else if ($status == 'X') $str = '<button class="" onClick="ChangeStatus(this, \'' . $mode . '\',\'A\',\'' . $id . '\'); return false;">E</button>';
		else $str = '<button class="" onClick="ChangeStatus(this, \'' . $mode . '\',\'A\',\'' . $id . '\'); return false;"><i class="fadeIn animated bx bx-x"></i></button>';
	} else {
		if ($status == "A") $str = '<span class="text-success"><i class="bx bx-check"></i></span>';
...
function I_LOGMASTER($u_id,$u_name,$mode,$flag,$name,$desc_str,$u_loc_id){
	$ip = $_SERVER['REMOTE_ADDR'];
	$lmid = NextID('iLMID', 'log_masters');
	$q = "insert into log_masters values ($lmid, $u_loc_id, $u_id, '" . db_input($u_name) . "', '" . NOW . "', '1', '$flag', '" . db_input($name) . "', '" . db_input($desc_str) . "', '$mode', '$ip', 'A')";
...
function LogMasterEdit2($id, $flag, $mode, $name = '', $desc_str = '', $user_id = false)
{
	...
	if ($desc_str != '') {
		...
		// INSERT INTO `hifigoa`.`log_masters` SET `iLMID`=1,`iLocID`=2,`iUserID`=2,`vUserName`='Darshan',`iRefID`=0,`cRefType`='0',`cStatus`='A';

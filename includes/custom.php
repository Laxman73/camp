...
function crm_workflow ($createrid,$typeid,$level,$division)
{
    // $stateid = GetXFromYID("select stateid from users 
    // inner join patches on patches.patchid=users.patch
    // where patches.patch_deleted=0 and users.deleted=0 and users.id=$createrid"); //Getting stateid


    //$division = GetXFromYID("select division from users where id='$createrid' "); //Getting division

    $_q = "select pending_with_id,status from crm_workflow
where typeid='$typeid' and division='$division'  and deleted = 0 and level = $level";
    $_r = sql_query($_q);
    list($pending_with_id, $status) = sql_fetch_row($_r);
    if ($pending_with_id > 0) {
        $ret=array('pending_with_id'=>$pending_with_id,'status'=>$status);
        return $ret;
    }else{
